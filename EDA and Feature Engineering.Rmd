...
## Feature selection
  ...
  When we choose the predictors that will be used in the model, we make a random forest-based selection to see the improtance level of all variables. We decide not to use the `Income_Level` since its iportance level is relatively low.
...
# Naive Bayes
  ...
  In this project, we have a lot of features that realtes to the matching between the therapy a patient is supposed to have and actually have. These features can be seen as given conditions, which are also known as prior information. So, Naive Bayesian classifier would be appropriate for this problem.
  
  By using `e1071` package, we have a initial Naive Bayesian model with an accuracy of 79.31%, which is slightly lower than the accuracy of the logistic (80.17%).
  
```{r echo=FALSE}
m2 <- naiveBayes(as.factor(Match) ~ Cancer_Stage  + Language.Isolation + X9th.Education + Bachelors.Education + Unemployed + Age.at.Diagnosis + Below.Poverty + Race + Sex + Insurance,data = trainset)
...
```
  ...
  Then we do the tuning of the `laplace` parameter, the red line indicates the accuracy of the initial model, and the blue line indicates the accuracy of the logistic model. When $laplace=70$, the model reached the highest accuracy. 
  
  However, it is still lower than the logistic model, which might due to the hypothesis of Naive Bayesian classifier that all variables' impact on the outcome is independent.
  
```{r echo=FALSE}
laplace_value <- seq(0,100,by=10)
train_err_bye <- rep(NA, length(laplace_value))

set.seed(1)
for (i in 1:length(laplace_value)) {
    bye_fold_model <-naiveBayes(as.factor(Match) ~ Cancer_Stage  + Language.Isolation + X9th.Education + Bachelors.Education + Unemployed + Age.at.Diagnosis + Below.Poverty + Race + Sex + Insurance,data = trainset,laplace = laplace_value[i])
    
    bye_fold_pred <- predict(bye_fold_model,testset)
